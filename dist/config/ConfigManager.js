...
class ConfigManager {
    ...
    validateRequired() {
        ...
        if (missing.length > 0) {
            console.error(`[ConfigManager] Missing required environment variables: ${missing.join(', ')}. ` +
                'Please set them in your .env file or environment before starting the bot.');
            process.exit(1);
        }
    }
    validateNumericThresholds() {
        const numericKeys = [
        ...
        if (errors.length > 0) {
            console.error(`[ConfigManager] Invalid numeric threshold configuration:\n  - ${errors.join('\n  - ')}`);
            process.exit(1);
        }
    }
    // ─── Helpers ───────────────────────────────────────────────────────────────
    ...
    parseString(key, defaultValue) {
        const raw = this.env[key];
        return raw?.trim() || defaultValue;
    }
    parseList(key, defaultValue) {
        const raw = this.env[key];
        if (!raw?.trim())
            return defaultValue;
        return raw
